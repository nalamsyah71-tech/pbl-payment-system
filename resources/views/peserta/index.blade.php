...
<div class="space-y-5" x-data="{ search: '' }">
    ...
    <div class="grid grid-cols-2 gap-3">
        <div class="bg-white border border-slate-100 rounded-xl p-4 shadow-sm">
            ...
            <p class="text-2xl font-extrabold text-slate-800">{{ $pesertas->total() }}</p>
        </div>
        <div class="bg-white border border-slate-100 rounded-xl p-4 shadow-sm">
            <p class="text-xs text-slate-500 font-medium">Halaman Ini</p>
            <p class="text-2xl font-extrabold text-slate-800">{{ $pesertas->count() }}</p>
        </div>
    </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="table-row hover:bg-indigo-50/20 transition-colors"
                        x-show="search === '' || '{{ strtolower($item->nik . ' ' . $item->nama . ' ' . ($item->kelas->nama_kelas ?? '')) }}'.includes(search.toLowerCase())">
                        <td class="px-5 py-3.5 text-slate-400 text-xs">
                            {{ ($pesertas->currentPage() - 1) * $pesertas->perPage() + $loop->iteration }}
                        </td>
                        <td class="px-5 py-3.5 font-mono text-xs font-semibold text-slate-600">{{ $item->nik }}</td>
                        ...
                        <td class="px-5 py-3.5 hidden md:table-cell">
                            <span class="badge bg-indigo-100 text-indigo-700">{{ $item->kelas->nama_kelas ?? '-' }}</span>
                        </td>
                        <td class="px-5 py-3.5 text-slate-500 text-xs hidden lg:table-cell">{{ $item->no_hp }}</td>
                        ...
                        <td class="px-5 py-3.5 text-center">
                            <div class="flex items-center justify-center gap-1">
                                ...
                                <form action="/peserta/{{ $item->id }}" method="POST"
                                      onsubmit="return confirm('Hapus peserta {{ $item->nama }}?')" class="inline">
                                    @csrf @method('DELETE')
                    ...
                    <tr>
                        <td colspan="7" class="px-5 py-16 text-center">
                            ...
                            <a href="{{ route('peserta.create') }}" class="inline-flex items-center gap-1 mt-3 text-sm text-indigo-600 font-semibold hover:text-indigo-700">
