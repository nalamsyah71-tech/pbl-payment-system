        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 card-hover group">
            <div class="flex items-start justify-between mb-3">
                ...
                <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <a href="{{ route('kejuruan.edit', $item) }}" class="p-1.5 text-indigo-600 hover:bg-indigo-50 rounded-lg">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                    </a>
                    <form action="{{ route('kejuruan.destroy', $item) }}" method="POST" onsubmit="return confirm('Hapus kejuruan ini?')">
                        @csrf @method('DELETE')
            ...
            @if($item->keterangan)
            <p class="text-xs text-slate-500 mt-1 line-clamp-2">{{ $item->keterangan }}</p>
            @endif
            <div class="mt-3 pt-3 border-t border-slate-100">
                <span class="text-xs text-slate-500">{{ $item->pelatihans_count ?? $item->pelatihans()->count() }} pelatihan</span>
            </div>
        </div>
        @empty
