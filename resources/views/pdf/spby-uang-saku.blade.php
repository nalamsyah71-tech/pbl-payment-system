...
<head>
    <meta charset="UTF-8">
    <title>SPPBy Uang Saku - {{ $pembayaranUangSaku->no_kuitansi }}</title>
    <style>
        body {
...
<body>
    <div class="header">
        ...
        <p>No. Kuitansi: {{ $pembayaranUangSaku->no_kuitansi }}</p>
    </div>
    ...
    <table class="info-table">
        <tr>
            <td>Tanggal SPBY:</td>
            <td>{{ \Carbon\Carbon::parse($pembayaranUangSaku->tgl_spby)->format('d/m/Y') }}</td>
        </tr>
        <tr>
            <td>Kelas:</td>
        ...
        <tr>
            ...
            <td class="text-right"><strong>Rp {{ number_format($pembayaranUangSaku->total_uang, 0, ',', '.') }}</strong></td>
        </tr>
    </table>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $peserta['nama'] ?? '-' }}</td>
                <td>{{ $peserta['no_hp'] ?? '-' }}</td>
                <td class="text-right">Rp {{ number_format($peserta['nominal'] ?? 0, 0, ',', '.') }}</td>
            </tr>
            @empty
            <tr>
