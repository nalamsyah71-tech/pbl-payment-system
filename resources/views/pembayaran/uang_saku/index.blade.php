    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
        ...
        <div class="flex flex-wrap items-end gap-3">
            ...
            <div class="flex items-center gap-1.5 bg-amber-50 border border-amber-200 text-amber-700 text-xs font-semibold px-3 py-2 rounded-xl">
                ...
                Filter aktif
                <a href="{{ route('pembayaran-uang-saku.index') }}" class="ml-1 text-amber-400 hover:text-amber-700">
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="flex items-center justify-between px-5 py-3.5 border-b border-slate-100 bg-slate-50/60">
            <span class="text-sm font-semibold text-slate-600">
                Daftar Pembayaran
                @if(request('kelas_id'))<span class="text-amber-500">· Kelas Terpilih</span>@endif
            </span>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-amber-50/30 transition-colors group">
                        ...
                        <td class="px-5 py-3.5 text-sm text-slate-500">{{ $item->kelas->pelatihan->nama ?? '-' }}</td>
                        <td class="px-5 py-3.5 text-right">
                            <span class="font-bold text-slate-800 text-sm">Rp {{ number_format($item->total_uang, 0, ',', '.') }}</span>
                        </td>
                        <td class="px-5 py-3.5 text-center">
                            <span class="text-xs text-slate-500 bg-slate-100 px-2.5 py-1 rounded-lg">
                                {{ $item->tgl_spby->format('d/m/Y') }}
                            </span>
                        </td>
                        <td class="px-5 py-3.5">
                            <div class="flex items-center justify-center gap-1">
                                <a href="{{ route('pembayaran-uang-saku.show', $item) }}"
                                ...
                                <a href="{{ route('pembayaran-uang-saku.pdf', $item) }}"
                                   class="p-2 text-red-500 hover:bg-red-50 hover:text-red-700 rounded-lg transition-all" title="Download PDF">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/>
                                    </svg>
                                </a>
                                <form action="{{ route('pembayaran-uang-saku.destroy', $item) }}" method="POST"
                    ...
                    <tr>
                        <td colspan="6" class="px-5 py-16 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <a href="{{ route('pembayaran-uang-saku.create') }}" class="text-sm text-amber-600 font-semibold hover:text-amber-700">
                                    + Tambah Sekarang
                                </a>
                            </div>
                        </td>
...
(function() {
    ...
    const pdfBase   = '{{ route("pembayaran-uang-saku.pdf-kelas", ["kelas" => "__ID__"]) }}';
    const excelBase = '{{ route("pembayaran-uang-saku.excel-kelas", ["kelas" => "__ID__"]) }}';
