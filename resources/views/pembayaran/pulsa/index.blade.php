@extends('layouts.app')
@section('title', 'Pembayaran Pulsa Internet')
@section('breadcrumb')
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
        ...
        <div class="flex flex-wrap items-end gap-3">
            ...
            <div class="flex items-center gap-1.5 bg-indigo-50 border border-indigo-200 text-indigo-700 text-xs font-semibold px-3 py-2 rounded-xl">
                ...
                Filter aktif
                <a href="{{ route('pembayaran-pulsa.index') }}" class="ml-1 text-indigo-400 hover:text-indigo-700">
                    ...
                    <tr class="hover:bg-indigo-50/30 transition-colors group">
                        ...
                        <td class="px-5 py-3.5">
                            <div class="flex items-center justify-center gap-1">
                                {{-- Detail --}}
                                <a href="{{ route('pembayaran-pulsa.show', $item) }}"
                                ...
                                {{-- PDF per item --}}
                                <a href="{{ route('pembayaran-pulsa.pdf', $item) }}"
                                ...
                                {{-- Hapus --}}
                                <form action="{{ route('pembayaran-pulsa.destroy', $item) }}" method="POST"
                    ...
                    <tr>
                        <td colspan="6" class="px-5 py-16 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <a href="{{ route('pembayaran-pulsa.create') }}" class="text-sm text-indigo-600 font-semibold hover:text-indigo-700">
                                    + Tambah Sekarang
                                </a>
                            </div>
                        </td>
...
(function() {
    ...
    const pdfBase   = '{{ rtrim(route("pembayaran-pulsa.pdf-kelas", ["kelas" => "__ID__"]), "") }}';
    const excelBase = '{{ rtrim(route("pembayaran-pulsa.excel-kelas", ["kelas" => "__ID__"]), "") }}';
