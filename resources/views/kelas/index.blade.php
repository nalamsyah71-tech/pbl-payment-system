@extends('layouts.app')
@section('title', 'Kelas')
@section('breadcrumb')
    <a href="{{ route('dashboard') }}" class="hover:text-gray-700">Dashboard</a>
    <span class="mx-2">/</span>
    <span class="text-gray-800 font-medium">Kelas</span>
@endsection
@section('content')
<div class="space-y-5">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-gray-900">Data Kelas</h1>
            <p class="text-sm text-gray-500 mt-0.5">Kelola data kelas pelatihan</p>
        </div>
        <a href="{{ route('kelas.create') }}" class="inline-flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
            Tambah Kelas
        </a>
    </div>
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-200 text-xs font-semibold text-gray-500 uppercase">
                    <th class="px-6 py-3 text-left">No</th>
                    <th class="px-6 py-3 text-left">Nama Kelas</th>
                    <th class="px-6 py-3 text-left">Pelatihan</th>
                    <th class="px-6 py-3 text-center">Peserta</th>
                    <th class="px-6 py-3 text-center">Periode</th>
                    <th class="px-6 py-3 text-center w-32">Aksi</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
                @forelse($kelas as $item)
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-3 text-gray-500">{{ $loop->iteration }}</td>
                    <td class="px-6 py-3 font-medium text-gray-900">{{ $item->nama_kelas }}</td>
                    <td class="px-6 py-3 text-gray-600">{{ $item->pelatihan->nama }}</td>
                    <td class="px-6 py-3 text-center">
                        <span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-green-100 text-green-700 text-xs font-bold">{{ $item->pesertas_count }}</span>
                    </td>
                    <td class="px-6 py-3 text-gray-500 text-xs">
                        {{ \Carbon\Carbon::parse($item->tgl_mulai)->format('d/m/Y') }} - {{ \Carbon\Carbon::parse($item->tgl_selesai)->format('d/m/Y') }}
                    </td>
                    <td class="px-6 py-3 text-center">
                        <div class="flex items-center justify-center gap-1">
                            {{-- SHOW - perbaiki parameter --}}
                            <a href="{{ route('kelas.show', ['kela' => $item->id]) }}" class="p-1.5 text-green-600 hover:bg-green-50 rounded-lg" title="Detail">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                            </a>
                            {{-- EDIT - perbaiki parameter --}}
                            <a href="{{ route('kelas.edit', ['kela' => $item->id]) }}" class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg" title="Edit">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </a>
                            {{-- DELETE - perbaiki parameter --}}
                            <form action="{{ route('kelas.destroy', ['kela' => $item->id]) }}" method="POST" onsubmit="return confirm('Hapus kelas {{ $item->nama_kelas }}?')">
                                @csrf @method('DELETE')
                                <button type="submit" class="p-1.5 text-red-500 hover:bg-red-50 rounded-lg" title="Hapus">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                </button>
                            </form>
                        </div>
                    </td>
                </tr>
                @empty
                <tr><td colspan="6" class="px-6 py-12 text-center text-gray-400">Belum ada data kelas.</td></tr>
                @endforelse
            </tbody>
        </table>
        @if($kelas->hasPages())
        <div class="px-6 py-4 border-t border-gray-100">{{ $kelas->links() }}</div>
        @endif
    </div>
</div>
@endsection