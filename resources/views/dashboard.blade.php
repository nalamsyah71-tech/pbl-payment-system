...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        ...
        <div class="bg-white rounded-2xl border border-slate-100 p-4 card-hover shadow-sm relative overflow-hidden">
            <div class="absolute -right-3 -top-3 w-16 h-16 rounded-full opacity-8 {{ $card['bg'] }}"></div>
            <div class="w-9 h-9 rounded-xl flex items-center justify-center mb-3 bg-gradient-to-br {{ $card['color'] }} shadow-sm">
                <svg class="w-4.5 h-[18px] text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $card['icon'] }}"/>
                </svg>
            </div>
            <p class="text-xs font-medium text-slate-500 mb-1 leading-tight">{{ $card['label'] }}</p>
            <p class="text-base font-extrabold text-slate-800 leading-tight truncate">{{ $card['value'] }}</p>
            <p class="text-xs {{ $card['text'] }} font-medium mt-0.5">{{ $card['unit'] }}</p>
        </div>
        @endforeach
    </div>

    {{-- Quick Access + Recent Table --}}
            ...
            <div class="space-y-2">
                ...
                <a href="{{ $qa['href'] }}"
                   class="flex items-center gap-3 bg-white border border-slate-100 rounded-xl p-3.5 card-hover shadow-sm group">
                    <div class="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 bg-gradient-to-br {{ $qa['color'] }} shadow-sm">
                        <svg class="w-[18px] h-[18px] text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $qa['icon'] }}"/>
                        </svg>
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold text-slate-800">{{ $qa['label'] }}</p>
                        <p class="text-xs text-slate-500">{{ $qa['sub'] }}</p>
                    </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-slate-50">
                            ...
                            <tr class="table-row hover:bg-slate-50">
                                <td class="px-4 py-3 font-mono text-xs font-semibold text-slate-700">{{ $payment['no_kuitansi'] }}</td>
                                <td class="px-4 py-3">
                                    <span class="badge {{ $tc['bg'] }} {{ $tc['text'] }}">{{ $payment['type'] }}</span>
                                </td>
                                <td class="px-4 py-3 text-slate-600 hidden sm:table-cell text-xs">{{ $payment['kelas'] }}</td>
                                <td class="px-4 py-3 text-right font-bold text-slate-800 text-xs">Rp {{ number_format($payment['total'],0,',','.') }}</td>
                                <td class="px-4 py-3 text-center text-xs text-slate-500 hidden md:table-cell">
                                    {{ \Carbon\Carbon::parse($payment['tgl'])->format('d/m/Y') }}
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
