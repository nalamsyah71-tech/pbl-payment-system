...
class PembayaranPulsaController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        // FIX: generate no_kuitansi dengan DB lock agar tidak bisa duplikat
        $no_kuitansi = DB::transaction(function () {
            $last = PembayaranPulsa::lockForUpdate()->orderBy('id', 'desc')->first();
            $lastNum = $last ? intval(substr($last->no_kuitansi, -4)) : 0;
            return 'PL/' . date('Ymd') . '/' . str_pad($lastNum + 1, 4, '0', STR_PAD_LEFT);
        });
    ...
    // ====== EDIT & UPDATE (BARU) ======
    public function edit(PembayaranPulsa $pembayaranPulsa): View
    {
        $pembayaranPulsa->load('kelas', 'peserta');
        $kelasList   = Kelas::with('pelatihan')->orderBy('nama_kelas')->get();
        $pesertaList = Peserta::where('kelas_id', $pembayaranPulsa->kelas_id)->orderBy('nama')->get();
        return view('pembayaran.pulsa.edit', compact('pembayaranPulsa', 'kelasList', 'pesertaList'));
    }

    public function update(Request $request, PembayaranPulsa $pembayaranPulsa): RedirectResponse
    {
        $request->validate([
            'kelas_id'   => 'required|exists:kelas,id',
            'peserta_id' => 'required|exists:pesertas,id',
        ...
        return redirect()->route('pembayaran-pulsa.show', $pembayaranPulsa)
            ->with('success', 'Data pembayaran pulsa berhasil diperbarui.');
    }
    ...
    public function pdfByKelas(Kelas $kelas)
    {
        ...
        $pembayaranList   = PembayaranPulsa::with('peserta')->where('kelas_id', $kelas->id)->orderBy('tgl_spby')->get();
                ...
                public function registerEvents(): array {
                    return [
                        \Maatwebsite\Excel\Events\AfterSheet::class => function (\Maatwebsite\Excel\Events\AfterSheet $e) {
                            ...
                            $sheet->setCellValue('G' . ($lastRow + 1), array_sum(array_column($this->data, 'nominal')));
